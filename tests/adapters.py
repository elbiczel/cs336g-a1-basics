...
def run_linear(
    ...
    """
    layer = nn.linear.Linear(d_in, d_out)
...
def run_embedding(
    ...
    layer = nn.embedding.Embedding(vocab_size, d_model)
    layer.load_state_dict({"weight": weights})
...
def run_swiglu(
    ...
    layer = nn.activation.SwiGLU(d_model, d_ff)
...
def run_scaled_dot_product_attention(
    ...
    """
    return nn.attention.attention(Q, K, V, mask)
...
def run_multihead_self_attention(
    ...
    """
    layer = nn.attention.MultiHeadSelfAttention(d_model, num_heads)
    layer.load_state_dict({
...
def run_transformer_block(
    ...
    layer.load_state_dict(adj_weights)
    return layer(in_features)
...
def run_rmsnorm(
    ...
    """
    layer = nn.layer_norm.RMSNorm(d_model, eps)
    layer.load_state_dict({"weight": weights})
    return layer(in_features)
...
def run_silu(in_features: Float[Tensor, " ..."]) -> Float[Tensor, " ..."]:
    ...
    """
    return nn.activation.silu(in_features)
...
def run_cross_entropy(
    ...
    """
    return nn.loss.cross_entropy(inputs, targets)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    return nn.optimizer.clip_grad(parameters, max_l2_norm)
...
def run_get_lr_cosine_schedule(
    ...
    """
    return nn.optimizer.cosine_lr_schedule(it, max_learning_rate, min_learning_rate, warmup_iters, cosine_cycle_iters)
...
def get_tokenizer(
    ...
    return bpe.MergeTokenizer(vocab, merges, byte_special_tokens)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    vocab, merges = bpe.train_bpe(input_path, vocab_size, special_tokens, num_processes=4)
